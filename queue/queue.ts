...
function initialize(): void {
  ...
  timeNextEvent[EventType.DEPARTURE] = 1.0e+30;
}
...
function report(): void {
  ...
  writeLine("\n\nAverage delay in queue: " +
    (totalDelays / numCustomersDelayed).toFixed(3) + " minutes");

  writeLine("Average number in queue: " +
    (areaNumInQueue / simTime).toFixed(3));

  writeLine("Server utilization: " +
    (areaServerStatus / simTime).toFixed(3));

  writeLine("Time simulation ended: " +
    simTime.toFixed(3) + " minutes");
}
...
try {
  ...
  if (params.length !== 3 || params.some(isNaN)) {
    console.error('Error: Input file must contain three numbers (mean interarrival time, mean service time, number of customers)');
    process.exit(1);
  }
  ...
  console.error('Please create a file named mm1.in with three space-separated values:');
